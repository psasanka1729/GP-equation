...
source_well_bias_potential_lst = np.around(np.linspace(12,20,16*2),2)
...
def transistor_potential_landscape(position_arr, SG_barrier_height, GD_barrier_height, gate_bias_potential,
     ...
     # Creating the source well.
     A = 0.04 # Increasing A results in increase in left side of the source well.
...
time_step_SI  = -1j*10**(-8) # In seconds unit.
...
final_time_SI = 35*10**(-3) # In seconds unit.
...
time_lst = list(np.arange(0,int(final_time_SI*1.e3),0.1))
