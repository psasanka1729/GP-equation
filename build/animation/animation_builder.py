...
#Do the git-ing
cmd='git commit -a -m "Commit before run animation" > /dev/null'
print(cmd)
...
open('animation.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('animation.task','w')
template_file='animation.template'
...
for L in range(64):
    ...
    task_file.write('bash animation_'+str(vnum)+'.qsub\n')
    fout.write(contents)
    fout.close()
...
contents=open('animation.sbatch.template','r').read()
...
open('animation.sbatch','w').write(contents)
