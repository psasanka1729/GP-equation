...
#Do the git-ing
cmd='git commit -a -m "Commit before run double_well_potential" > /dev/null'
print(cmd)
...
open('double_well_potential.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('double_well_potential.task','w')
template_file='double_well_potential.template'
...
contents=open('double_well_potential.sbatch.template','r').read()
...
open('double_well_potential.sbatch','w').write(contents)
