...
#Do the git-ing
cmd='git commit -a -m "Commit before run imaginary_time_evolution" > /dev/null'
print(cmd)
...
open('imaginary_time_evolution.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('imaginary_time_evolution.task','w')
template_file='imaginary_time_evolution.template'
...
for L in range(64):
    ...
    task_file.write('bash imaginary_time_evolution_'+str(vnum)+'.qsub\n')
    fout.write(contents)
    fout.close()
...
contents=open('imaginary_time_evolution.sbatch.template','r').read()
...
open('imaginary_time_evolution.sbatch','w').write(contents)
