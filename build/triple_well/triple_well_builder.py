...
#Do the git-ing
cmd='git commit -a -m "Commit before run triple_well" > /dev/null'
print(cmd)
...
open('triple_well.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('triple_well.task','w')
template_file='triple_well.template'
...
for L in range(48):
    ...
    task_file.write('bash triple_well_'+str(vnum)+'.qsub\n')
    fout.write(contents)
    fout.close()
...
contents=open('triple_well.sbatch.template','r').read()
...
open('triple_well.sbatch','w').write(contents)
