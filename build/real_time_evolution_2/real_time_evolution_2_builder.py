...
#Do the git-ing
cmd='git commit -a -m "Commit before run real_time_evolution_2" > /dev/null'
print(cmd)
...
open('real_time_evolution_2.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('real_time_evolution_2.task','w')
template_file='real_time_evolution_2.template'
...
for L in xrange(16):
	...
	task_file.write('bash real_time_evolution_2_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('real_time_evolution_2.sbatch.template','r').read()
...
open('real_time_evolution_2.sbatch','w').write(contents)
