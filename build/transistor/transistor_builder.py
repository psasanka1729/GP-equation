...
#Do the git-ing
cmd='git commit -a -m "Commit before run transistor" > /dev/null'
print(cmd)
...
open('transistor.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('transistor.task','w')
template_file='transistor.template'
...
for L in range(64):
    ...
    task_file.write('bash transistor_'+str(vnum)+'.qsub\n')
    fout.write(contents)
    fout.close()
...
contents=open('transistor.sbatch.template','r').read()
...
open('transistor.sbatch','w').write(contents)
