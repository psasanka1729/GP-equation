...
#Do the git-ing
cmd='git commit -a -m "Commit before run coherence" > /dev/null'
print(cmd)
...
open('coherence.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('coherence.task','w')
template_file='coherence.template'
...
for L in range(256):
	...
	task_file.write('bash coherence_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('coherence.sbatch.template','r').read()
...
open('coherence.sbatch','w').write(contents)
